...
# Navigate to the project directory: platformio.ini is one level up from here,
# and `pio run` only works from inside the project directory.
cd "$(dirname "$0")/.." || exit
...
for ENV in "${ENV_LIST[@]}"; do
    ...
    if [ $? -eq 0 ]; then
        ...
        echo "platformio device monitor -p $ESP_DEVICE -b 921600"
        ...
        if [[ $REPLY =~ ^[Yy]$ ]]; then
            ...
            platformio device monitor -p "$ESP_DEVICE" -b 921600
        fi
