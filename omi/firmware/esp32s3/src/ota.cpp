...
// Mozilla root CA bundle embedded by arduino-esp32. Validating against it is
// what makes an OTA URL trustworthy; the alternative, setInsecure(), lets anyone
// who can answer the DNS name install arbitrary firmware.
...
static bool download_and_install_firmware()
{
    ...
    if (isHttps) {
        ...
        // Fail closed: a certificate we cannot verify aborts the download rather
        // than downgrading to an unauthenticated one.
